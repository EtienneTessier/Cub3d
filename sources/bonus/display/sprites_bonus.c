...
static void	load_enemy(t_sprite *sprite, t_ray ray, t_data *data)
{
	...
	while (sprite->stripe < sprite->draw_end_x)
	{
				...
				SCR_WIDTH && sprite->transform_y < ray.z_buffer[sprite->stripe])
		{
			...
			{
				ray.tex_y = ((y - SCR_HEIGHT / 2 + sprite->height / 2) * \
					BEN_HEIGHT) / sprite->height;
				if (ray.txr[ray.tex_y * BEN_WIDTH + ray.tex_x] != 0xff0000)
					my_mlx_pixel_put(data->img, sprite->stripe, y, ray.txr \
						[ray.tex_y * BEN_WIDTH + ray.tex_x]);
				y++;
			}
		}
		sprite->stripe += 1;
...
void	print_sprites(t_data *data, t_ray ray, t_player player)
{
	...
	while (++i < data->ennemis_count)
	{
		sprite->distance[i] = ((player.x - data->ennemis[i].x) * (player.x - \
			data->ennemis[i].x) + (player.y - data->ennemis[i].y) * (player.y - \
			data->ennemis[i].y));

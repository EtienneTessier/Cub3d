...
// 	sprite->transform_x = sprite->inv_det * (data->player.dir_y * sprite->x - \
// 		data->player.dir_x * sprite->y);
// 	sprite->transform_y = sprite->inv_det * (-data->player.plan_y * sprite->x \
// 		+ data->player.plan_x * sprite->y);
// 	sprite->screen_x = (int)((SCR_WIDTH / 2) * (1 + sprite->transform_x / \
// 		sprite->transform_y));
// }
static void	find_enemy_pos(int i, t_data *data, t_sprite *sprite)
{
...
// 	{
// 		ray.tex_x = (int)(256 * (sprite->stripe - (-sprite->width / 2 + \
// 			sprite->screen_x)) * BEN_WIDTH / sprite->width) / 256;
// 		sprite->draw_y = sprite->draw_start_y;
// 		if (sprite->transform_y > 0 && sprite->stripe > 0 && sprite->stripe < \
// 				SCR_WIDTH && sprite->transform_y < ray.z_buffer[sprite->stripe])
// 		{
// 			while (sprite->draw_y < sprite->draw_end_y)
// 			{
// 				ray.tex_y = ((sprite->draw_y - SCR_HEIGHT / 2 + sprite->height \
// 					/ 2) * BEN_HEIGHT) / sprite->height;
// 				if (ray.txr[ray.tex_y * BEN_WIDTH + ray.tex_x] != NOT_PIXEL)
// 					my_mlx_pixel_put(data->img, sprite->stripe, sprite->draw_y, \
// 						ray.txr[ray.tex_y * BEN_WIDTH + ray.tex_x]);
// 				sprite->draw_y++;
// 			}
// 		}
// 		sprite->stripe += 1;
